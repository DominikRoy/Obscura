...
sys.path.append('/home/kamil/.local/lib/python3.9/site-packages')
...
"""
The code is from https://github.com/nann-cheng/CondEval, it was adapted to fit in the scenario of obscura.
"""
class Server:
    ...
    def getFirstRoundMessageDrone(self):
        ...
        vv = self.innerProduct(self.circuit["non_vec_v"],self.circuit["non_vec_v"])
        vvRets = ring_add(vv,self.circuit["vv_out"],SEMI_HONEST_MODULO)
        ...
        #vvRets = self.innerProductWithMul(["in_v", "in_v", "v_v", "vv_out"])
        #masked_value = vec_add(self.circuit["non_vec_v"], self.circuit["in_v_drone"], SEMI_HONEST_MODULO)
        return[ipRets, ssRets, vvRets, masked_value]
        #return[ipRets, ssRets, vvRets
    ...
    for index in range(BENCHMARK_TESTS_AMOUNT):
       ...
       # computation_start_time = time.time()
        ...
        if _id == 0:
            ...
            firstround = time.time() - computation_start_time
            all_computation_time += firstround
            comm = time.time()
            pool.asend("server", mShares)
            otherShares = await pool.recv("server")
            ...
            vvWire = mShares[2]#ring_add(mShares[2], otherShares[2], SEMI_HONEST_MODULO)
            all_computation_time += time.time() - computation_start_time
            ...
            all_comm+= time.time()-comm
            computation_start_time = time.time()
            mShares = server.getFirstRoundMessage(otherShares[3])
            firstround = time.time() - computation_start_time
            all_computation_time += firstround
            comm = time.time()
            pool.asend("server", mShares)
            ...
            ipWire = ring_add(mShares[0], otherShares[0], SEMI_HONEST_MODULO)
            ssWire = ring_add(mShares[1], otherShares[1], SEMI_HONEST_MODULO)
            vvWire = otherShares[2]#ring_add(mShares[2], otherShares[2], SEMI_HONEST_MODULO)
            all_computation_time += time.time() - computation_start_time
        ...
        #ipWireout = ring_add(mShares[3], otherShares[3], SEMI_HONEST_MODULO)
        #ssWireout = ring_add(mShares[4], otherShares[4], SEMI_HONEST_MODULO)
        #vvWireout = ring_add(mShares[5], otherShares[5], SEMI_HONEST_MODULO)
        ...
        #all_online_computation_time += time.time() - computation_start_time
        all_online_computation_time += all_computation_time
        #all_online_time += time.time() - start_time
        all_online_time += online_time
        
        print("Comp.Time:",all_computation_time)
        print("Comm.Time:",all_comm)
        print("Firstround:",firstround)
        print("OnlineTime:",online_time)
        print("CompCommTime:", all_computation_time+all_comm)
