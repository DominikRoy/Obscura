...
def get_energy_consumption(hostname, username, password, remote_path,local, running_time) -> pd.DataFrame:
    ...
    try:
        ...
        with open(local, 'r') as file:
            ...
            mask = df[6].str.contains(r'^\[PID \d+\] python3\.9', regex=True)
...
def execute_command_parallel_alternative_v2(hosts, usernames, passwords, commands, timeout=200):
    if len(hosts) != len(usernames) or len(hosts) != len(passwords) or len(hosts) != len(commands):
        raise ValueError("The number of hosts, usernames, passwords, and commands must be equal.")

    # Create host configuration for each host
    host_configs = [
        HostConfig(port=22, user=usernames[i], password=passwords[i])
        for i in range(len(hosts))
    ]
    # Create a list of command pairs for each host
    #command_pairs = [(commands[0], commands[1]) for _ in hosts]

    client = ParallelSSHClient(hosts=hosts, host_config=host_configs, timeout=timeout)
    
    # Run commands for each host
    output = client.run_command('%s',host_args=(commands[0], commands[1]), use_pty=True, sudo=True, read_timeout=120)
    line_host = []
    line_host1 = []
    #for host_out in output:
    print(f"Output from {output[0].host}:")
    for line in output[0].stdout:
           line_host.append(line)
           print(f"[{output[0].host}] {line.strip()}")
    for line in output[0].stderr:
           print(f"[{output[0].host} ERROR] {line.strip()}")

    print(f"Output from {output[1].host}:")
    for line in output[1].stdout:
           line_host1.append(line)
           print(f"[{output[1].host}] {line.strip()}")
    for line in output[1].stderr:
           print(f"[{output[1].host} ERROR] {line.strip()}")
    
    #client.join(output)
    return [line_host,line_host1]



def execute_command_parallel_alternative_v3(hosts, usernames, passwords, commands, timeout=200):
    if len(hosts) != len(usernames) or len(hosts) != len(passwords) or len(hosts) != len(commands):
        raise ValueError("The number of hosts, usernames, passwords, and commands must be equal.")
    ...
    # Run commands for each host
    output = client.run_command('%s',host_args=(commands[0], commands[1]), use_pty=True, sudo=True, read_timeout=120)
    #output = client.run_command(commands, use_pty=True, sudo=True, read_timeout=120)
    line_host = []
    line_host1 = []
    #for host_out in output:
    print(f"Output from {output[0].host}:")
